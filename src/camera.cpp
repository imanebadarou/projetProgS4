...
glm::mat4 Camera::getViewMatrix() const {
    if (mode == CameraMode::Trackball) {
        ...
        return glm::lookAt(subjectivePos, subjectivePos + front, glm::vec3(0.0f, 1.0f, 0.0f));
    }
}
...
void Camera::rotateTrackball(float deltaTheta, float deltaPhi) {
    if (mode != CameraMode::Trackball) return;
...
void Camera::setSubjectivePosition(glm::vec3 pos) {
    subjectivePos = pos;
}
